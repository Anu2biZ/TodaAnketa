...
function checkAuthorization() {
  ...
  console.log('Статус авторизации:', 
    authStatus === ScriptApp.AuthorizationStatus.REQUIRED ? 'Требуется авторизация' :
  ...
  console.log('\nТекущие настройки:');
  console.log('- SPREADSHEET_ID:', properties.SPREADSHEET_ID || 'не указан');
  console.log('- ONBOARDING_FOLDER_ID:', properties.ONBOARDING_FOLDER_ID || 'не указан');
  console.log('- RECIPIENT_EMAIL:', properties.RECIPIENT_EMAIL || 'не указан');
  ...
  try {
    ...
    if (properties.SPREADSHEET_ID) {
      ...
      console.log('✓ Google Sheets: доступ получен, таблица:', ss.getName());
    } else {
      console.log('❌ Google Sheets: не проверено (ID не указан)');
    }
    ...
    if (properties.ONBOARDING_FOLDER_ID) {
      ...
      console.log('✓ Google Drive: доступ получен, папка:', folder.getName());
    } else {
      console.log('❌ Google Drive: не проверено (ID не указан)');
    }
    
    // Проверяем Gmail
    const quota = MailApp.getRemainingDailyQuota();
    console.log('✓ Gmail: доступ получен, доступная квота:', quota);
  } catch (e) {
    console.error('❌ Ошибка при проверке доступа:', e.message);
  }
...
function setup() {
  ...
  try {
    ...
    console.log('SPREADSHEET_ID:', properties.SPREADSHEET_ID);
    ...
    console.log('ONBOARDING_FOLDER_ID:', properties.ONBOARDING_FOLDER_ID);
    ...
    if (!properties.RECIPIENT_EMAIL) {
      ...
      console.log('Email получателя указан:', properties.RECIPIENT_EMAIL);
    }
